...
def test_mongodb_variable_cache():
    ...
    variable_value = '100'
    mongo = MongoDBUtil()
    mongo.set_variable_value(process_instance_id, variable_key, variable_value)

    value = mongo.get_variable_value(process_instance_id, variable_key, 'str')
    assert value == '100'

    value = mongo.get_variable_value(process_instance_id, variable_key, 'int')
    assert value == 100

    variable_key = 'test002'
    variable_value = {'name': 'zs'}
    mongo.set_variable_value(process_instance_id, variable_key, variable_value)
    value = mongo.get_variable_value(process_instance_id, variable_key, 'dict')
    assert value == {'name': 'zs'}

    variable_key = 'test003'
    variable_value = ['name', 'age']
    mongo.set_variable_value(process_instance_id, variable_key, variable_value)
    value = mongo.get_variable_value(process_instance_id, variable_key, 'list')
    assert value == ['name', 'age']
    ...
    variable_value = pd.DataFrame(data)
    mongo.set_variable_value(process_instance_id, variable_key, variable_value)
    value = mongo.get_variable_value(process_instance_id, variable_key, 'pd.dataframe')
    assert isinstance(value, pd.DataFrame)
